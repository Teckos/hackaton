...
app.get('/db/:id', async (req,res) => {
    ...
    const selectById = "SELECT * FROM test WHERE id = ?";
    try {
        const result = await prepStatementModule.executeQuery(selectById,[id]);
        res.status(200).json(result);
    } catch(err) {
        console.log(err)
        res.status(500).send('Something happened...');
...
app.post('/db', async(req, res) => {
    const insertQuery = "INSERT INTO test (label, valeur) VALUES (?, ?)";
    const values = ['Randlde', 22];
    try {
